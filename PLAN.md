...
- 建立 `AGENTS.md`（开发规范：每阶段必须提交 git、更新 PLAN.md、只增不改、禁止项、构建测试命令）。
- 建立 `PLAN.md`（阶段总览、阶段 5 逐项状态、验证记录、遗留问题优先级）。
- `AGENTS.md` 增补“文件结构（入库）”“`include/core/` 关键头文件职责”“新增代码放置规则”。
- 本轮扁平导出接口文档 `docs/interface-for-analysis.md` 与样例配置 `config/flat-demo.ini` 一并入库。
...
| 优先级 | 项 | 说明 | 状态 |
|---|---|---|---|
| P1 | `flags` 列承载方式 | 规则 3 需 `off_schedule` 标记，A1 26 列无处承载；已作为末列追加 | 待确认（默认保留） |
| P1 | `unknown_schedule` 行为变化 | 未申报周期的参数观测状态由 `ok` 变为 `unknown_schedule`（规则 1） | 需知会下游（文档已写） |
| P2 | `bus_analyze --export-flat` 别名 | 可选便利入口，需“逐字节等价”测试断言 | 未做 |
| P2 | 本地演示件 | `presentation/*` 与 `run-full-demo.bat` | **已处理：明确加入 `.gitignore`（不入库）** |
| P3 | `available_time_ns` 离线代理值 | 离线取 `ingest` 代理，文档注明为保守上界 | 已记录 |
| P3 | 环境依赖 | 构建需关闭“智能应用控制”，且该开关不可逆 | 已记录 |
| P3 | 槽位/观测双行 | 被满足的到期槽位会与观测行各出一行（规则 4） | 已按规则并写入文档 |

### P1 处理建议
- `flags`：默认**保留末列方案**；若下游更希望合并进 `status` 或改用布尔列，改动仅限导出层，代价小。
- `unknown_schedule`：下游读取时按“无申报周期=不作缺失判定”处理，不要把它当作异常。
