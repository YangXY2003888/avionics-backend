...
`[fusion <id>]` 用 `count` 和 `source_i`/`parameter_i` 声明 2~16 路来源，配置 `method`、`tolerance` 和 `window_ms`。每当某一路产生新样本时，规则取其余各路在参考时刻之前、窗口内最近的可用样本完成对齐，要求同一时间组且新鲜。当前按最近样本保持对齐，没有线性插值。

- `mean`：各值算术平均；
- `median`：各值中位数；
- `vote`：各值四舍五入后取众数，票数持平不产生结果。

各值极差（最大减最小）在 `tolerance` 内输出 `fused`，metric 为融合值；否则输出 `fused_divergence`，metric 仍为融合值供参考。两者都保留全部参与通道作为证据。融合值是分析结论，不会被写回参数流。
